...
effective for quickly creating publication quality graphics in a flexible way.
...
par( mar=c(5,5,1,6)+0.1, las=1, xpd=T )
plot( lifeExp ~ year, gapminder, col=as.numeric(continent) )
...
```{r}
ggplot(gapminder, aes(gdpPercap, lifeExp)) +
  geom_point(size = 3, colour = "goldenrod1") + 
  scale_x_log10() +
  geom_smooth(method = "lm", size = 1.5,col='dodgerblue')
```
